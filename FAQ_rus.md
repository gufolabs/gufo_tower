...
**О**: Есть несколько вещей которые дополнительно делаются для инсталляции типа Prod.
  ...
  * Копируется файл для совместной работы supervisord и alerta
  * Патчится supervisord на предмет поддержки oom_score_adj на linux системах. Патч основан на https://github.com/Supervisor/supervisor/pull/754

Для инсталяций типа eval и test локальные правки блокируют обновление. Локальные правки должны храниться в отдельной директории custom. Структура этого каталога должна повторять структуру в основном репозитарии. Локальные комимты для этих типов инсталяции будут отклонены. 

Инсталяция типа develop после начальной инсталяции совсем не обновляет дерево исходных кодов. Только выполняет команду `git fetch origin`. В инсталяции этого типа предполгается что все дальнейшие работы будут выполняться администратором вручную. Эта версия толерантно относится к переключению ветвей и накладыванию любых правок
...
Debian 9 на текущий момент не поддерживается. Mongodb пока не выпустила пакетов под Debian 9. Ждем.
...
Для инсталляции в докере убедиться, что выполнены (рекомендации)[https://groups.google.com/forum/#!topic/ansible-project/y8ohlv_dRi4] 
...
**О**: Используя консоль можно более точно управлять, что именно и как именно должно происходить. К примеру:
...
**О**: В основном репозитарии [тут](https://code.getnoc.com/noc/ansible_deploy). Однако скачиваются с зеркала на [github](https://github.com/nocproject/ansible_deploy) 

**В**: Я поставил по инструкции и все проверил, однако при инсталляции у меня ошибка. Где спросить помощи?

**О**: Можно тут https://t.me/nocproject
...
```
Дальше по обстоятельсвам. Чаще всего это означает, что слетела авторизация в mongo.
...
* `alpine` - на основе alpine linux
* остальные. как правило, согласно выпущенным версиям.

Для установки можно использовать такой примерно `docker-compose.yml`
```
version: '2'
services:
  tower:
 ...
 * Update Sources - выполняет `git pull` в директории noc и в директории custom
 * Update configs - конфигурирует все сервисы.
 * Install requirements - выполняет команды аналогичные ./bin/pip install -r requirements/noc.txt. Бывает полезно при впиливании каких то новых зависимостей.
...
Для автоматизации рутинных операций есть возможность запустить playbook для разбивки. Сделать это можно только из консоли сервера. Веб интерфейс для этих задач не предусмотрен.
...
Если же предполагается использовать другую файловую систему, перед запуском плейбука нужно отредактировать файл 
...
```
***В***: Я запустил деплой на хосты которые запущены внутри KVM\VmWare\etc виртуальной машины и деплой останавливается на проверке SSE 4.2 хотя на хост машине он поддерживается.

***O***: Необходимо в настройках контейнера выставить "копировать тип CPU с хост машины" или версию CPU с поддержкой этого расширения. Тип KVM ***не*** поддерживает эмуляцию
SSE 4.2!!! ***Остановите контейнер и запустите его заново!!!*** смена типа CPU ***не*** происходит при перезагрузке контейнера.