...
**О**: На текущий момент отличие только одно. Инсталляция типа `Prod` заботится о поддержании в чистоте каталога `/opt/noc`.
*Все файлы, которые не внесены в репозитарий, удаляются.* Если Это поведение выглядит неразумным, используйте альтернативные варианты типов.
...
**В**: Что означают опции развертывния.

**О**: После начальной инсталяции, как правило полный накат всего playbook не нужен, достаточно обновить исходники и рестартануть сервис.
 ...
 * Update Sources - выполняет `hg pull -u` в диреткории noc и в директории custom
 ...
 * Install requirements - выполняет команды аналогичные ./bin/pip install -r requirements/noc.txt. Бывает полезно при впиливании каких то новых фич.
 ...
 Дальше идут подряд два варинта перезапуска NOC после применения `playbook`
 ...
 * Restart gentle - Делает рестарт средствами `supervisord`. Аналогично команде `./noc ctl serialrestart *`.
 Этот вариант перезапуска рекомендуется для крупных инсталяций, когда серисов NOC больше, чем по одному. Если выбраны оба варианта пожежбает этот.
